...
**Scaffold the seven files with `npm run generate:module`** (`scripts/generate-module.ts`)
instead of hand-copying `user/`. Give it a Prisma model to read the field list from, or
run it with no arguments for a generic Task CRUD starting point:

```
npm run generate:module -- --schema prisma/schema.prisma --model <ModelName>
npm run generate:module                     # no schema: generic Task CRUD
```

It only writes under `src/modules/<name>/` — it never touches `schema.prisma`,
permissions, `routes/index.ts`, or `openapi.ts`, and prints ready-to-paste snippets for
those instead. It also does not resolve relations into joins: a foreign-key scalar
(`assigneeId`) comes through as a plain field, the relation object (`assignee`) is
dropped — add joins by hand the same as any other generated module.
