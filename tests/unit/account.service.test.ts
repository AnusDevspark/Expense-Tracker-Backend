...
describe('AccountService', () => {
  ...
  describe('getAccountById', () => {
    it('throws NotFoundError when the record does not exist', async () => {
      ...
      await expect(service.getAccountById('missing-id', owner)).rejects.toThrow(NotFoundError);
    });

    it('returns the mapped record when found', async () => {
      repository.findById.mockResolvedValue(makeAccountRecord());
      const result = await service.getAccountById('11111111-1111-4111-8111-111111111111', owner);
  ...
  describe('createAccount', () => {
    it('writes exactly the whitelisted fields', async () => {
      ...
      await service.createAccount(sampleInput, owner);
  ...
  describe('updateAccount', () => {
    it('throws NotFoundError when the record does not exist', async () => {
      repository.findById.mockResolvedValue(null);
      await expect(service.updateAccount('missing-id', sampleInput, owner)).rejects.toThrow(NotFoundError);
    ...
    it('writes exactly the whitelisted fields when the record exists', async () => {
      ...
      await service.updateAccount('11111111-1111-4111-8111-111111111111', sampleInput, owner);
  ...
  describe('deleteAccount', () => {
    it('throws NotFoundError when the record does not exist', async () => {
      repository.findById.mockResolvedValue(null);
      await expect(service.deleteAccount('missing-id', owner)).rejects.toThrow(NotFoundError);
